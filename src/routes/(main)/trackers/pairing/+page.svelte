<script lang="ts">
	...
	let unsubscribeDeviceDiscovered: (() => void) | null = null;
	...
	onMount(async () => {
		try {
			...
			unsubscribeDeviceDiscovered = await listen("haritora_device_discovered", (event: any) => {
				const device = event.payload as DiscoveredDevice;
				info(`Discovered device: ${device.name} (${device.address})`);
	...
	onDestroy(() => {
		if (unsubscribeDeviceDiscovered) unsubscribeDeviceDiscovered();
